...
test.describe('Size layout test cases', () => {
    ...
    test('should verify Size Section functionality', async ({ page }) => {
        // Check nevigation to Store/Size page
        await navigateMenu(page, 'Store', 'Library', 'Size');
        // Check the page header Store/Size
        const pageHeader = page.getByRole('heading', { name: 'Store/Size' });
        ...
        // Name input
        await fillFilterInput(filterPanel, '1 feet', 0);
        // Remarks input
    ...
    test('should verify Size Dashboard functionality', async ({ page }) => {
        // Check nevigation to Store/Size page
        await navigateMenu(page, 'Store', 'Library', 'Size');
        // Check the page header Store/Size
        const pageHeader = page.getByRole('heading', { name: 'Store/Size' });
        ...
        // Wait for Update Size Info page to load
        const updateTitle = page.getByText('Update Size', { exact: true });
        await expect(updateTitle).toBeVisible({ timeout: TEST_TIME['2min'] });
        //Fill "Name" field
        await fillInputByLabel(page, /name/i, '30-44 inches');
        //Fill "Remarks" field
        await fillInputByLabel(page, /remarks/i, 'Test Remarks');
        // Click Save button
        await clickSaveOrSubmitButton(page, TEST_TIME['2min']);
    });
    test('should verify add Size functionality', async ({ page }) => {
        // Check nevigation to Store/Size page
        await navigateMenu(page, 'Store', 'Library', 'Size');
        // Check the page header Store/Size
        const pageHeader = page.getByRole('heading', { name: 'Store/Size' });
        ...
        // Wait for Add Size Info page to load
        const addPageTitle = page.getByText('Add Size', { exact: true });
        await expect(addPageTitle).toBeVisible({ timeout: TEST_TIME['2min'] });
        //Fill "Name" field
        await fillInputByLabel(page, /name/i, '20 cm');
        //Fill "Remarks" field
        await fillInputByLabel(page, /remarks/i, 'Test Remarks');
        // Click Save button
        await clickSaveOrSubmitButton(page, TEST_TIME['2min']);
